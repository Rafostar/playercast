...
function onPlayerCast(msg)
{
	...
	controller.load(err =>
	{
		if(!err)
		{
			...
			return websocket.emitEvent('status-update', status);
		}
...
function handlePlayerLaunch()
{
	...
	controller.on('playback-started', afterMediaLoad);
	...
	controller.once('app-exit', (code) =>
	{
		...
		controller.removeListener('playback-started', afterMediaLoad);
...
{
	if(debug.enabled)
		debug(`Cast message: ${JSON.stringify(msg)}`);
	...
	switch(opts.app)
	{
		case 'mpv':
			var mpvUniversal = ['--fullscreen', '--volume-max=100',
				...
				`--force-media-title=${status.title}`];
...
function afterMediaLoad(isLoaded)
{
	if(!isLoaded) return;
